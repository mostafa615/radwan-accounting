...
@section('content')
    ...
    <div class="row">
        <form role="form" class="validate" action="{{route('orders-in.store')}}" method="POST"
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="mandator_id" id="mandator_id">
                                        <option value="">لايوجد</option>
                                        @foreach ($mandators as $mandator)
                                            <option value="{{$mandator->id}}">{{$mandator->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <!-- <input type="button" value="Attack" onclick="refreshClients();"> -->
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" min="0" step="any" value="0" required class="form-control observe"
                                            id="old">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="reposite_id" id="reposite_id">
                                        @foreach ($reposites as $reposite )
                                            ...
                                            <option value="{{$reposite->id}}">{{$reposite->name}} @if($reposite->branch_id == auth()->user()->branch_id) (خزنة الفرع) @endif</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.disabled == true) {
                        $('#unite_price').attr('disabled', true);
                        $('#unite_price').prop('disabled', true); //true|disabled will work on all
                        $('#unite_price').attr('disabled', true); 
                        $('#unite_price').disabled = true;
                    }else{
                        $('#unite_price').attr('disabled', false);
                        $('#unite_price').prop('disabled', false); //false|disabled will work on all
                        $('#unite_price').attr('disabled', false); 
                        $('#unite_price').disabled = false;
                    }
                    ...
                    // المقاسات الخاصة لوحدها 2181 صنف بأسماء متشابهة،
                    // فبحث بالاسم أأمن بكتير من التمرير في القائمة.
                    initItemSearch();
                }
            })
...
//                    $('#quantity').val(data.data.quantity);
        ...
        $('#add').on('click', function () {
            ...
            if (store_id == 0 || store_id == null || store_id == '') {
                $('#errors').show();
                $('#errors').delay(1500).fadeOut(350);
                return false;
            }
            ...
            $('<tr>').html(
                ...
                '<td><input type="text"  class="form-control text-center" data-id="' + item_id + '_' + store_id + '_' + group_id + '" readonly  onchange="update_quantity($(this).data(\'id\'))" name="price[]" value="' + price + '" id="' + item_id + '_' + store_id + '_' + group_id + '_price' + '"></td>' +
        ...
        $('#driving_cost').on('change', function () {
            ...
            if( $('#old').val() < $('#driving_cost').val() )  {
                result  = $('#driving_cost').val() - $('#old').val();
                
                final_total = parseFloat(final_total) + result;
                
            }
            if( $('#old').val() > $('#driving_cost').val()  ) {
                
                result  = $('#old').val() - $('#driving_cost').val();
