    ...
    <div class="box box-primary">
      <div class="box-header with-border">
        ...
        <div class="box-btn">
          ...
          <a class="btn btn-success  btn-sm btn-flat" href="{{route('return-orders-out.create')}}">
            اضافة
          </a>
          @endif
        </div>
          ...
          <div class="row">
            <div class="col-xs-6">
              <div class="col-lg-3 col-md-3 col-sm-12" style="margin-top: 20px;">
                <div class="form-group">
                  <?php
                    $pendingOrders = App\Models\Order::where('is_return', true)->where("ownerable_type", "App\Models\Supplier")->where('type', 'out')
              ...
              <div class="col-lg-3 col-md-3 col-sm-12" style="margin-top: 20px;">
                <div class="form-group">
                  <?php
                    $refusedOrders = App\Models\Order::where('is_return', true)->where("ownerable_type", "App\Models\Supplier")->where('type', 'out')
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  <a href="{{url('return-orders-out-store-license/'.$resource->id)}}" target="_blank" class="btn btn-success" title="إذن صرف من المخزن"><i class="fa fa-print"></i></a>
                  <a href="{{url('return-orders-out-license/'.$resource->id)}}" class="btn btn-instagram" target="_blank" title="إذن"><i class="fa fa-print"></i></a>
                  <a href="{{url('return-orders-out/'.$resource->id)}}" class="btn btn-info" target="_blank"><i class="fa fa-print"></i></a>
                  <!--@endif-->
                  ...
                  @if(Auth::user()->hasRole('admin') || Auth::user()->branch_id == $resource->branch_id)
                    @if(Auth::user()->can('edit_order') && Auth::user()->has_edit_orders == 1)
                      @if(\App\Models\Order::where('id', $resource->id)->whereHas('orderDetails', function ($query) {
                        ...
                        <a href="{{url('return-orders-out/'.$resource->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                      @endif
                    @endif
                    ...
                    @if(Auth::user()->can('delete_order'))
                      @if(\App\Models\Order::where('id', $resource->id)->whereHas('orderDetails', function ($query) {
                        ...
                        @if($resource->cost <= 0)
                        <button class="btn btn-danger" data-toggle="modal" data-target="#delete_{{$resource->id}}">
                        ...
                        <div id="delete_{{$resource->id}}" class="modal fade" role="dialog">
                          <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                {{Form::open(['route'=>['return-orders-out.destroy',$resource->id],'method'=>'DELETE'])}}
