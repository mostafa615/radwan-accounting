...
@section('sub-title','تقرير حالة المبيعات ')
@section('content')
    <div class="row">
            ...
            <div class="box-body">
                <table class="table table-bordered" id="example_10" >
                    ...
                    <tr>
                        ...
                        @endforeach
                        <td>الاجمالي </td>
                        <td> مبيعات الصنف من ({{ $date_from }}) </td>
                    ...
                    @foreach($resources as $resource)
                        <tr>
                            ...
                            @foreach($stores as $store )
                                ...
                                $itemCountSold += DB::table('order_details')
                                                ->join('orders', 'orders.id', '=', 'order_details.order_id')
                                                ->where('order_details.item_id',$resource->id)->where('order_details.store_id',$store->id)->where('orders.ownerable_type','App\Models\Client')
                                                ->where('order_details.created_at', '>=', $date_from)
                                                ->sum('quantity');
                               ?>
