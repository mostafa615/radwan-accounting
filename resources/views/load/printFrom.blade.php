    ...
    <div class="box">
        ...
        <table class="table table-responsive table-bordered">
          ...
          <tr>
            ...
            <td>
              <?php  
                try{
                  echo App\Models\Quantity::with('ownerable')->where('ownerable_id', $resource->From->id)->where('ownerable_type','App\Models\Store')->where('item_id',$item->item->id)->where('quantity','>=',1)->first()->quantity; 
                }catch(\Exception $e){
                  echo 0;
                }
              ?>
      ...
      <div class="box-body">
        <table class="table table-bordered table-striped text-center">
          ...
          <tbody>
            @forelse($resource->sendEmployees as $employee)
            <tr>
              <td>{{$employee->name}}</td>
            </tr>
            @empty
      ...
      <div class="box-footer">
        ...
        <label class="col-md-3">{{{$resource->sender->name or '-'}}}</label>
      </div>
    </div>
  </div>
