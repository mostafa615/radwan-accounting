...
class ReturnOrdersOutController extends Controller
{
    ...
    // {
    //     $resources = Order::with('orderDetails', 'ownerable', 'reposite', 'user')->where('is_return', true)->where("ownerable_type", "App\Models\Supplier")->where('type', 'out')->orderBy('created_at', 'DESC')->get();
    //     return view('return-orders-out.index', compact('resources'));
    // }
    
    public function index(ReturnOrdersOutDataTable $dataTable) {
        $query = Order::with('orderDetails', 'ownerable', 'reposite', 'user')->where('is_return', true)->where("ownerable_type", "App\Models\Supplier")->where('type', 'out');
    ...
    public function create()
    {
        ...
        /*Supplier::whereHas('quantities', function ($query) {
...
//         ]);
//         $this->util->activityLog(auth()->user()->id, 'create', 'accounts', 'return-orders-out', $order->id, $this->dateNow, $this->timeNow, null, null );

//         $client = Supplier::where('id', $request->supplier_id)->first();
//         if ($client) {
//             $client->decrement('balance', ($request->final_total - $request->cost));
//         }
...
//     }
    ...
    public function store(Request $request) {
        ...
        foreach ($request->group_id as $item) {
            ...
            $group = DB::table('groups')->where('id', $item)->first();
            // if($group->name == "NO4"){
            //     $supply = Supply::find($request->item_id[$counter]);
            //     if($supply->quantity < $request->quantity[$counter]){
            //         flash('الكمية المستلزم المطلوبة أكبر من المتاحة في المخزن')->error();
            //         return back();
            //     }
            //     $order->is_oper_supplies=1;
            //     $order->save();
            ...
            if ($group->name == "NO4") {
                $order->is_oper_supplies = 1;
                $order->save();
                $supply = DB::table('supplies')->where('id', $request->item_id[$counter])->first();
                if ($supply->quantity < $request->quantity[$counter]) {
                    flash('كمية المستلزم المطلوبة أكبر من المتاحة في المخزن')->error();
                    return back();
                }
            }
    ...
    public function show($id) {
        ...
        return view('return-orders-out.show', compact('resource', 'type'));
    }
    ...
    public function print_store_license(int $id) {
        $type = 'store';
        $resource = Order::with('orderDetails', 'ownerable', 'reposite')->findOrFail($id);
        return view('return-orders-out.storeShow', compact('resource', 'type'));
    }

    public function edit($id)
    { 
        $resource = Order::with('orderDetails', 'ownerable')->where('type', 'out')->where('is_return', true)->where('id', $id)->first();
        
        return view('return-orders-out.edit', compact('resource'));
    }
    ...
    //     $this->util->activityLog(auth()->user()->id, 'update', 'accounts', 'return-orders-out', $resource->id, $this->dateNow, $this->timeNow, $properties, null );
    ...
    public function update(Request $request, $id) {
        ...
        $resource = Order::where('id', $id)->first();
        ...
        $this->util->activityLog(auth()->user()->id, 'update', 'accounts', 'return-orders-out', $resource->id, $this->dateNow, $this->timeNow, $properties, null );
        if($pendingRefuedOrders) {
            $this->push_notification_update(['user_id' => $resource->receiver_id, 'url' => url('return-orders-out'), 'message' => 'تم اجراء تعديل فى فاتورة رقم ' . $resource->id]);
        }
        DB::commit();
        flash('تم الحفظ بنجاح')->success();
        return redirect()->route('return-orders-out.index');
    ...
    public function destroy(Order $order)
    {
        ...
        $oldData = Order::where('id', $order->id)->first();
        $properties = [
            'old_data' => $oldData,
        ];
        $this->util->activityLog(auth()->user()->id, 'delete', 'accounts', 'return-orders-out', $order->id, $this->dateNow, $this->timeNow, $properties, null );
