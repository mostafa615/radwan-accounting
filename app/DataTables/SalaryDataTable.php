...
//            dd($basic);
            ...
            ->addColumn('loans', function (Employee $employee) {
                ...
                ->whereMonth('date', Carbon::parse($this->request()->date)->month)
                ->whereYear('date', Carbon::parse($this->request()->date)->year)
                ->first();
                
                
                $loans = $employee->solaf_balance + optional($salary)->loans ;
                // dd($employee->solaf_balance);
                $id = $employee->id;
...
//                }
            ...
            ->addColumn('transports', function (Employee $employee) {
                $transports =$employee->getTransportsSum($this->request()->date);
                $id = $employee->id;
            ...
            ->addColumn('insurance', function (Employee $employee) {
                ...
                $insurance = $employee->social_insurance + $employee->medical_insurance;
                $id = $employee->id;
                // dd($employee);
                return view('salary.datatbles.insurance', compact('id', 'insurance'))->render();
            })
            ->addColumn('net', function (Employee $employee) {
                return "<button type='button' class='btn btn-primary fa fa-eye btn-sm calcNet' onclick='calculateNet(this.parentElement)' ></button><span class='net' ></span>";//optional($employee->salaries->first())->net . "<button type='button' class='btn btn-primary fa fa-eye btn-sm' onclick='calculateNet(this.parentElement)' ></button><span class='net' ></span>";
            })
            ->addColumn('late', function (Employee $employee) {

                return round($employee->late($this->request()->date),2);
            })
            ->addColumn('over_time', function (Employee $employee) {

                return round($employee->overTime($this->request()->date),2);
            })
            ->addColumn('over_time_cost', function (Employee $employee) {
                ...
                return "<span class='over-time-cost' >" . round($hour_cost * $employee->overTime($this->request()->date),2) . "</span>";
            ...
            ->addColumn('late_cost', function (Employee $employee) {
                ...
                $workHours = AttendanceSettings::first()->work_hours;
                $hour_cost = $employee->salary / ($workDays * $workHours);
                return "<span class='late-cost' >" . round($hour_cost * $employee->late($this->request()->date),2) . "</span>";

            })
            ->addColumn('absence_cost', function (Employee $employee) {
                $workDays = AttendanceSettings::first()->work_days;
                $day_cost = $employee->salary / ($workDays); 
                return "<span class='absence-cost' >" . round($day_cost * $employee->absence($this->request()->date),2) . "</span>";

            })
            ->addColumn('total_registered_days', function (Employee $employee) {
                return $employee->attendancesPerMonth($this->request()->date);
            })
            ->addColumn('action', function (Employee $employee) {
                $salaryExist = false;
                $id = $employee->id;
                $salary = optional($employee->salaries->first());
                if(!is_null($salary->date))
                    $salaryExist = true;

                $date = $this->request()->date;
                return view('salary.datatbles.actions', compact('id', 'salary', 'date','salaryExist'))->render(); 
            })
            ...
            ->rawColumns(['basic', 'madionia', 'loans', 'bonus', 'transports', 'financial_penalties', 'insurance', 'net','reposite_id', 'action', 'late_cost','over_time','over_time_cost', 'absence_cost', 'notes']);
    }
    ...
    public function query(Employee $model)
    {
        ...
        $query = $model
            ...
            ->select(
                ...
                'jobs.name as job',
                'employees.salary',
                'employees.medical_insurance',
                'employees.social_insurance',
    ...
    protected function getColumns()
    {
        return [
            ...
            [
                'name' => 'late',
                'data' => 'late',
                'title' => 'تاخير وانصراف (س)',
                'orderable' => false,
                'searchable' => false,
            ],[
                'name' => 'late_cost',
                'data' => 'late_cost',
                'title' => 'تاخبر وانصراف (ج)',
                'orderable' => false,
                'searchable' => false,
            ],
            [
                'name' => 'over_time',
                'data' => 'over_time',
                'title' => 'وقت اضافي ( ساعة )',
                'orderable' => false,
                'searchable' => false,
            ],[
                'name' => 'over_time_cost',
                'data' => 'over_time_cost',
                'title' => 'وقت اضافي ( جنيه )',
                'orderable' => false,
                'searchable' => false,
            ],
