...
class RepositesDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        ->editColumn('mainly',function(Reposite $item){
            if($item->mainly)
                return "ثابتة";
            else
                return "متغيرة";
    ...
    {
        $query =  $model->select('reposites.mainly','reposites.priority','reposites.created_at','reposites.id','reposites.name','balance','users.name as user','branches.name as branch')
        ...
        if (auth()->user()->id != 1) {
                $query->where('reposites.branch_id', auth()->user()->branch_id);
        }
